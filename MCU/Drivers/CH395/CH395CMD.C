...
void CH395StartSendingData(UINT8 sockindex, UINT16 total_len)
{
	xWriteCH395Cmd(CMD30_WRITE_SEND_BUF_SN);
	xWriteCH395Data((UINT8)sockindex);
	xWriteCH395Data((UINT8)total_len);
	xWriteCH395Data((UINT8)(total_len>>8));
}

void CH395ContinueSendingData(UINT8 *databuf, UINT16 len)
{
	xWriteCH395Bulk(databuf, len);
}
void CH395Complete()
{
	xEndCH395Cmd();
