EESchema Schematic File Version 4
EELAYER 30 0
EELAYER END
$Descr A4 11693 8268
encoding utf-8
Sheet 3 4
Title "6x2 Antenna Switch Control/Interlock Board - Remote Control"
Date "2019-04-13"
Rev "0.1"
Comp "Lu, Zening"
Comment1 "BI4WOP"
Comment2 ""
Comment3 ""
Comment4 ""
$EndDescr
Text Label 2550 4250 0    50   ~ 0
SPI_SCK
Text Label 2550 4350 0    50   ~ 0
SPI_MISO
Text Label 2550 4450 0    50   ~ 0
SPI_MOSI
Text Label 2550 3950 0    50   ~ 0
TXD
Text Label 2550 4050 0    50   ~ 0
RXD
Text Label 2800 5850 0    50   ~ 0
SWDIO
Text Label 2750 5950 0    50   ~ 0
SWCLK
Wire Wire Line
	2800 5850 3000 5850
Wire Wire Line
	3000 5950 2700 5950
Wire Wire Line
	3000 5750 2950 5750
Wire Wire Line
	2950 5750 2950 5650
$Comp
L power:+3.3V #PWR048
U 1 1 5EF36463
P 2950 5650
F 0 "#PWR048" H 2950 5500 50  0001 C CNN
F 1 "+3.3V" H 2965 5823 50  0000 C CNN
F 2 "" H 2950 5650 50  0001 C CNN
F 3 "" H 2950 5650 50  0001 C CNN
	1    2950 5650
	1    0    0    -1  
$EndComp
$Comp
L Connector_Generic:Conn_01x04 J6
U 1 1 5EF38A09
P 3200 5850
F 0 "J6" H 3280 5842 50  0000 L CNN
F 1 "SWD" H 3280 5751 50  0000 L CNN
F 2 "Pin_Headers:Pin_Header_Straight_1x04_Pitch2.54mm" H 3200 5850 50  0001 C CNN
F 3 "~" H 3200 5850 50  0001 C CNN
	1    3200 5850
	1    0    0    -1  
$EndComp
Wire Wire Line
	3000 6050 2950 6050
Wire Wire Line
	2950 6050 2950 6100
$Comp
L power:GND #PWR052
U 1 1 5EF3C191
P 2950 6100
F 0 "#PWR052" H 2950 5850 50  0001 C CNN
F 1 "GND" H 2955 5927 50  0000 C CNN
F 2 "" H 2950 6100 50  0001 C CNN
F 3 "" H 2950 6100 50  0001 C CNN
	1    2950 6100
	1    0    0    -1  
$EndComp
Text Label 7500 3000 0    50   ~ 0
SPI_SCK
Text Label 7500 3100 0    50   ~ 0
SPI_MOSI
Text Label 7500 3200 0    50   ~ 0
SPI_MISO
Wire Wire Line
	7500 3100 7450 3100
Wire Wire Line
	7500 3200 7450 3200
Wire Wire Line
	7500 3000 7450 3000
$Comp
L power:GND #PWR040
U 1 1 5EF64C56
P 8000 2700
F 0 "#PWR040" H 8000 2450 50  0001 C CNN
F 1 "GND" H 8150 2650 50  0000 C CNN
F 2 "" H 8000 2700 50  0001 C CNN
F 3 "" H 8000 2700 50  0001 C CNN
	1    8000 2700
	1    0    0    -1  
$EndComp
Wire Wire Line
	7900 3500 7900 5150
Wire Wire Line
	4850 1450 4850 1550
Wire Wire Line
	4850 3400 5950 3400
Wire Wire Line
	5050 3300 5050 1700
Wire Wire Line
	5050 3300 5950 3300
Wire Wire Line
	5950 3200 5150 3200
Wire Wire Line
	5150 3200 5150 1800
Wire Wire Line
	5350 3100 5950 3100
Wire Wire Line
	5850 1450 5850 2300
Wire Wire Line
	5850 2500 5950 2500
$Comp
L Device:R R46
U 1 1 5EFCD804
P 5750 1700
F 0 "R46" H 5820 1746 50  0000 L CNN
F 1 "470" H 5820 1655 50  0000 L CNN
F 2 "lc_lib:0603_R" V 5680 1700 50  0001 C CNN
F 3 "~" H 5750 1700 50  0001 C CNN
	1    5750 1700
	1    0    0    -1  
$EndComp
Wire Wire Line
	5750 1450 5750 1550
$Comp
L power:+3.3V #PWR037
U 1 1 5EFD0601
P 5750 2150
F 0 "#PWR037" H 5750 2000 50  0001 C CNN
F 1 "+3.3V" H 5900 2200 50  0000 C CNN
F 2 "" H 5750 2150 50  0001 C CNN
F 3 "" H 5750 2150 50  0001 C CNN
	1    5750 2150
	0    -1   -1   0   
$EndComp
Wire Wire Line
	5550 1450 5550 1800
$Comp
L power:GND #PWR032
U 1 1 5EFDBD32
P 5550 1800
F 0 "#PWR032" H 5550 1550 50  0001 C CNN
F 1 "GND" H 5555 1627 50  0000 C CNN
F 2 "" H 5550 1800 50  0001 C CNN
F 3 "" H 5550 1800 50  0001 C CNN
	1    5550 1800
	1    0    0    -1  
$EndComp
Wire Wire Line
	7450 2600 7850 2600
Text Label 800  2450 0    50   ~ 0
RST
Wire Wire Line
	5950 2800 5850 2800
$Comp
L Device:C_Small C32
U 1 1 5F00C352
P 800 2800
F 0 "C32" H 892 2846 50  0000 L CNN
F 1 "104" H 892 2755 50  0000 L CNN
F 2 "lc_lib:0603_C" H 800 2800 50  0001 C CNN
F 3 "~" H 800 2800 50  0001 C CNN
	1    800  2800
	-1   0    0    -1  
$EndComp
$Comp
L Device:R_Small R50
U 1 1 5F01B20A
P 1100 2800
F 0 "R50" H 1041 2754 50  0000 R CNN
F 1 "47K" H 1041 2845 50  0000 R CNN
F 2 "lc_lib:0603_R" H 1100 2800 50  0001 C CNN
F 3 "~" H 1100 2800 50  0001 C CNN
	1    1100 2800
	1    0    0    1   
$EndComp
$Comp
L Interface_UART:MAX485E U6
U 1 1 5F032C9E
P 4200 6650
F 0 "U6" H 4000 7250 50  0000 C CNN
F 1 "MAX485E" H 4000 7150 50  0000 C CNN
F 2 "digikey-footprints:SOIC-8_W3.9mm" H 4200 5950 50  0001 C CNN
F 3 "https://datasheets.maximintegrated.com/en/ds/MAX1487E-MAX491E.pdf" H 4200 6700 50  0001 C CNN
	1    4200 6650
	1    0    0    -1  
$EndComp
Wire Wire Line
	2800 5850 2800 5050
Wire Wire Line
	2700 5950 2700 5150
$Comp
L Device:C_Small C34
U 1 1 5F0D3010
P 5100 3700
F 0 "C34" V 4871 3700 50  0000 C CNN
F 1 "10pF" V 4962 3700 50  0000 C CNN
F 2 "lc_lib:0603_C" H 5100 3700 50  0001 C CNN
F 3 "~" H 5100 3700 50  0001 C CNN
	1    5100 3700
	0    1    1    0   
$EndComp
$Comp
L Device:C_Small C35
U 1 1 5F0D770F
P 5100 4000
F 0 "C35" V 5200 4000 50  0000 C CNN
F 1 "10pF" V 5300 4000 50  0000 C CNN
F 2 "lc_lib:0603_C" H 5100 4000 50  0001 C CNN
F 3 "~" H 5100 4000 50  0001 C CNN
	1    5100 4000
	0    1    1    0   
$EndComp
Connection ~ 5000 3850
Wire Wire Line
	5000 3850 4900 3850
Wire Wire Line
	4900 3850 4900 3900
$Comp
L power:GND #PWR047
U 1 1 5F0E65A6
P 3250 5400
F 0 "#PWR047" H 3250 5150 50  0001 C CNN
F 1 "GND" H 3255 5227 50  0000 C CNN
F 2 "" H 3250 5400 50  0001 C CNN
F 3 "" H 3250 5400 50  0001 C CNN
	1    3250 5400
	1    0    0    -1  
$EndComp
$Comp
L Device:R_Small R51
U 1 1 5F0EB528
P 5800 4100
F 0 "R51" H 5741 4054 50  0000 R CNN
F 1 "12K" H 5741 4145 50  0000 R CNN
F 2 "lc_lib:0603_R" H 5800 4100 50  0001 C CNN
F 3 "~" H 5800 4100 50  0001 C CNN
	1    5800 4100
	-1   0    0    1   
$EndComp
Wire Wire Line
	5950 3900 5800 3900
Wire Wire Line
	5800 3900 5800 4000
$Comp
L power:GND #PWR045
U 1 1 5F0F19B6
P 5800 4300
F 0 "#PWR045" H 5800 4050 50  0001 C CNN
F 1 "GND" H 5805 4127 50  0000 C CNN
F 2 "" H 5800 4300 50  0001 C CNN
F 3 "" H 5800 4300 50  0001 C CNN
	1    5800 4300
	1    0    0    -1  
$EndComp
Wire Wire Line
	5800 4200 5800 4300
$Comp
L power:GND #PWR046
U 1 1 5F0F6A72
P 6700 4900
F 0 "#PWR046" H 6700 4650 50  0001 C CNN
F 1 "GND" H 6705 4727 50  0000 C CNN
F 2 "" H 6700 4900 50  0001 C CNN
F 3 "" H 6700 4900 50  0001 C CNN
	1    6700 4900
	1    0    0    -1  
$EndComp
Wire Wire Line
	6700 4750 6700 4900
Wire Wire Line
	5750 1850 5750 2150
$Comp
L Device:R R47
U 1 1 5F14B91E
P 6050 1700
F 0 "R47" H 6120 1746 50  0000 L CNN
F 1 "470" H 6120 1655 50  0000 L CNN
F 2 "lc_lib:0603_R" V 5980 1700 50  0001 C CNN
F 3 "~" H 6050 1700 50  0001 C CNN
	1    6050 1700
	1    0    0    -1  
$EndComp
Wire Wire Line
	6050 1550 6050 1450
$Comp
L power:+3.3V #PWR038
U 1 1 5F1512A6
P 6050 2150
F 0 "#PWR038" H 6050 2000 50  0001 C CNN
F 1 "+3.3V" H 6200 2250 50  0000 C CNN
F 2 "" H 6050 2150 50  0001 C CNN
F 3 "" H 6050 2150 50  0001 C CNN
	1    6050 2150
	0    -1   -1   0   
$EndComp
Wire Wire Line
	6050 1850 6050 2150
Wire Wire Line
	7450 3500 7900 3500
Wire Wire Line
	3800 6750 3750 6750
Wire Wire Line
	3750 6750 3750 6650
Wire Wire Line
	3750 6650 3800 6650
Connection ~ 3750 6650
Text Label 3500 6650 0    50   ~ 0
485RW
Wire Wire Line
	3500 6650 3750 6650
Wire Wire Line
	3800 6550 3450 6550
Wire Wire Line
	3800 6850 3450 6850
Wire Wire Line
	4200 6150 4200 6050
$Comp
L power:+3.3V #PWR050
U 1 1 5F1C820C
P 4200 5800
F 0 "#PWR050" H 4200 5650 50  0001 C CNN
F 1 "+3.3V" H 4215 5973 50  0000 C CNN
F 2 "" H 4200 5800 50  0001 C CNN
F 3 "" H 4200 5800 50  0001 C CNN
	1    4200 5800
	1    0    0    -1  
$EndComp
$Comp
L Device:C_Small C36
U 1 1 5F1C90A4
P 4400 6050
F 0 "C36" V 4171 6050 50  0000 C CNN
F 1 "104" V 4262 6050 50  0000 C CNN
F 2 "lc_lib:0603_C" H 4400 6050 50  0001 C CNN
F 3 "~" H 4400 6050 50  0001 C CNN
	1    4400 6050
	0    1    1    0   
$EndComp
Wire Wire Line
	4300 6050 4200 6050
Connection ~ 4200 6050
Wire Wire Line
	4200 6050 4200 5800
$Comp
L power:GND #PWR051
U 1 1 5F1D7230
P 4550 6050
F 0 "#PWR051" H 4550 5800 50  0001 C CNN
F 1 "GND" H 4555 5877 50  0000 C CNN
F 2 "" H 4550 6050 50  0001 C CNN
F 3 "" H 4550 6050 50  0001 C CNN
	1    4550 6050
	1    0    0    -1  
$EndComp
Wire Wire Line
	4500 6050 4550 6050
$Comp
L power:GND #PWR056
U 1 1 5F1DD087
P 4200 7300
F 0 "#PWR056" H 4200 7050 50  0001 C CNN
F 1 "GND" H 4205 7127 50  0000 C CNN
F 2 "" H 4200 7300 50  0001 C CNN
F 3 "" H 4200 7300 50  0001 C CNN
	1    4200 7300
	1    0    0    -1  
$EndComp
Wire Wire Line
	4200 7250 4200 7300
Text Label 3450 6550 0    50   ~ 0
RXD
Text Label 3450 6850 0    50   ~ 0
TXD
$Comp
L Device:R_Small R53
U 1 1 5F1EC256
P 4800 6450
F 0 "R53" H 4859 6496 50  0000 L CNN
F 1 "4.7K" H 4859 6405 50  0000 L CNN
F 2 "lc_lib:0603_R" H 4800 6450 50  0001 C CNN
F 3 "~" H 4800 6450 50  0001 C CNN
	1    4800 6450
	1    0    0    -1  
$EndComp
$Comp
L Device:R_Small R54
U 1 1 5F1EF55F
P 4800 7000
F 0 "R54" H 4859 7046 50  0000 L CNN
F 1 "4.7K" H 4859 6955 50  0000 L CNN
F 2 "lc_lib:0603_R" H 4800 7000 50  0001 C CNN
F 3 "~" H 4800 7000 50  0001 C CNN
	1    4800 7000
	1    0    0    -1  
$EndComp
Wire Wire Line
	4600 6850 4800 6850
Wire Wire Line
	4800 6850 4800 6900
Wire Wire Line
	4600 6550 4800 6550
$Comp
L power:+3.3V #PWR055
U 1 1 5F1F6237
P 4700 7100
F 0 "#PWR055" H 4700 6950 50  0001 C CNN
F 1 "+3.3V" H 4715 7273 50  0000 C CNN
F 2 "" H 4700 7100 50  0001 C CNN
F 3 "" H 4700 7100 50  0001 C CNN
	1    4700 7100
	1    0    0    -1  
$EndComp
Wire Wire Line
	4700 7100 4800 7100
$Comp
L power:GND #PWR053
U 1 1 5F1FAD9A
P 4700 6400
F 0 "#PWR053" H 4700 6150 50  0001 C CNN
F 1 "GND" H 4705 6227 50  0000 C CNN
F 2 "" H 4700 6400 50  0001 C CNN
F 3 "" H 4700 6400 50  0001 C CNN
	1    4700 6400
	1    0    0    -1  
$EndComp
Wire Wire Line
	4700 6400 4700 6350
Wire Wire Line
	4700 6350 4800 6350
Connection ~ 4800 6550
Connection ~ 4800 6850
$Comp
L Connector:Screw_Terminal_01x03 J7
U 1 1 5F20EA36
P 5950 6700
F 0 "J7" H 6030 6742 50  0000 L CNN
F 1 "485" H 6030 6651 50  0000 L CNN
F 2 "Connector_Phoenix_MSTB:PhoenixContact_MSTBA_2,5_3-G-5,08_1x03_P5.08mm_Horizontal" H 5950 6700 50  0001 C CNN
F 3 "~" H 5950 6700 50  0001 C CNN
	1    5950 6700
	1    0    0    1   
$EndComp
Wire Wire Line
	5750 6600 5450 6600
Wire Wire Line
	5450 6600 5450 6550
Wire Wire Line
	5450 6850 5450 6800
Wire Wire Line
	5450 6800 5750 6800
$Comp
L power:GND #PWR054
U 1 1 5F2197A6
P 5650 7000
F 0 "#PWR054" H 5650 6750 50  0001 C CNN
F 1 "GND" H 5655 6827 50  0000 C CNN
F 2 "" H 5650 7000 50  0001 C CNN
F 3 "" H 5650 7000 50  0001 C CNN
	1    5650 7000
	1    0    0    -1  
$EndComp
Wire Wire Line
	5750 6700 5650 6700
Wire Wire Line
	5650 6700 5650 7000
Text GLabel 1350 4150 0    50   Output ~ 0
BCD1_0
$Comp
L Device:C_Small C33
U 1 1 5F2D6FD7
P 5850 2900
F 0 "C33" H 5942 2946 50  0000 L CNN
F 1 "104" H 5942 2855 50  0000 L CNN
F 2 "lc_lib:0603_C" H 5850 2900 50  0001 C CNN
F 3 "~" H 5850 2900 50  0001 C CNN
	1    5850 2900
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR041
U 1 1 5F2DC47A
P 5650 2950
F 0 "#PWR041" H 5650 2700 50  0001 C CNN
F 1 "GND" H 5655 2777 50  0000 C CNN
F 2 "" H 5650 2950 50  0001 C CNN
F 3 "" H 5650 2950 50  0001 C CNN
	1    5650 2950
	1    0    0    -1  
$EndComp
Wire Wire Line
	5650 2950 5750 2950
Wire Wire Line
	5750 2950 5750 3000
Wire Wire Line
	5750 3000 5850 3000
$Comp
L Device:C_Small C9
U 1 1 5F316A1A
P 7700 1000
F 0 "C9" H 7550 1050 50  0000 L CNN
F 1 "104" H 7550 950 50  0000 L CNN
F 2 "lc_lib:0603_C" H 7700 1000 50  0001 C CNN
F 3 "~" H 7700 1000 50  0001 C CNN
	1    7700 1000
	1    0    0    -1  
$EndComp
Wire Wire Line
	7700 900  7700 800 
Connection ~ 7700 800 
Wire Wire Line
	7700 800  7750 800 
$Comp
L power:GND #PWR026
U 1 1 5F31EB8D
P 7700 1150
F 0 "#PWR026" H 7700 900 50  0001 C CNN
F 1 "GND" H 7705 977 50  0000 C CNN
F 2 "" H 7700 1150 50  0001 C CNN
F 3 "" H 7700 1150 50  0001 C CNN
	1    7700 1150
	1    0    0    -1  
$EndComp
Wire Wire Line
	7700 1100 7700 1150
Wire Wire Line
	8600 900  8600 800 
$Comp
L Device:C_Small C10
U 1 1 5F33012C
P 8600 1000
F 0 "C10" H 8688 1046 50  0000 L CNN
F 1 "10uF" H 8688 955 50  0000 L CNN
F 2 "lc_lib:0805_C" H 8600 1000 50  0001 C CNN
F 3 "~" H 8600 1000 50  0001 C CNN
	1    8600 1000
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR028
U 1 1 5F330852
P 8600 1150
F 0 "#PWR028" H 8600 900 50  0001 C CNN
F 1 "GND" H 8605 977 50  0000 C CNN
F 2 "" H 8600 1150 50  0001 C CNN
F 3 "" H 8600 1150 50  0001 C CNN
	1    8600 1150
	1    0    0    -1  
$EndComp
Wire Wire Line
	8600 1100 8600 1150
$Comp
L power:GND #PWR027
U 1 1 5F33A081
P 8050 1200
F 0 "#PWR027" H 8050 950 50  0001 C CNN
F 1 "GND" H 8055 1027 50  0000 C CNN
F 2 "" H 8050 1200 50  0001 C CNN
F 3 "" H 8050 1200 50  0001 C CNN
	1    8050 1200
	1    0    0    -1  
$EndComp
$Comp
L power:+1V8 #PWR024
U 1 1 5F3437E3
P 8600 800
F 0 "#PWR024" H 8600 650 50  0001 C CNN
F 1 "+1V8" H 8615 973 50  0000 C CNN
F 2 "" H 8600 800 50  0001 C CNN
F 3 "" H 8600 800 50  0001 C CNN
	1    8600 800 
	1    0    0    -1  
$EndComp
Connection ~ 8600 800 
$Comp
L power:+1V8 #PWR034
U 1 1 5F347E3E
P 6850 1850
F 0 "#PWR034" H 6850 1700 50  0001 C CNN
F 1 "+1V8" H 6800 2000 50  0000 C CNN
F 2 "" H 6850 1850 50  0001 C CNN
F 3 "" H 6850 1850 50  0001 C CNN
	1    6850 1850
	1    0    0    -1  
$EndComp
$Comp
L power:+3.3V #PWR023
U 1 1 5F389B23
P 7200 800
F 0 "#PWR023" H 7200 650 50  0001 C CNN
F 1 "+3.3V" H 7215 973 50  0000 C CNN
F 2 "" H 7200 800 50  0001 C CNN
F 3 "" H 7200 800 50  0001 C CNN
	1    7200 800 
	1    0    0    -1  
$EndComp
Wire Wire Line
	5000 3850 5000 4000
$Comp
L Device:Crystal_GND24 Y1
U 1 1 60528F3B
P 5400 3850
F 0 "Y1" V 5050 3800 50  0000 L CNN
F 1 "30MHz" V 5150 3750 50  0000 L CNN
F 2 "Crystals:Crystal_SMD_3225-4pin_3.2x2.5mm" H 5400 3850 50  0001 C CNN
F 3 "~" H 5400 3850 50  0001 C CNN
	1    5400 3850
	0    1    1    0   
$EndComp
Wire Wire Line
	5700 4000 5700 3800
Wire Wire Line
	5700 3800 5950 3800
Wire Wire Line
	5000 3700 5000 3850
$Comp
L MySchLib:CH395Q U4
U 1 1 5EF04DC7
P 6150 2350
F 0 "U4" H 7050 -50 50  0000 C CNN
F 1 "CH395Q" H 7050 50  50  0000 C CNN
F 2 "lc_lib:LQFP-64_10X10X05P" H 6150 2350 50  0001 C CNN
F 3 "" H 6150 2350 50  0001 C CNN
	1    6150 2350
	1    0    0    -1  
$EndComp
$Comp
L Device:C_Small C13
U 1 1 606284EA
P 7050 1750
F 0 "C13" H 6900 1800 50  0000 L CNN
F 1 "104" H 6900 1700 50  0000 L CNN
F 2 "lc_lib:0603_C" H 7050 1750 50  0001 C CNN
F 3 "~" H 7050 1750 50  0001 C CNN
	1    7050 1750
	1    0    0    1   
$EndComp
$Comp
L power:+3.3V #PWR033
U 1 1 6063D50A
P 6550 1850
F 0 "#PWR033" H 6550 1700 50  0001 C CNN
F 1 "+3.3V" H 6500 2000 50  0000 C CNN
F 2 "" H 6550 1850 50  0001 C CNN
F 3 "" H 6550 1850 50  0001 C CNN
	1    6550 1850
	1    0    0    -1  
$EndComp
Wire Wire Line
	6850 1850 6850 1950
$Comp
L Device:C_Small C26
U 1 1 5CF3BE9F
P 7050 2050
F 0 "C26" H 6900 2100 50  0000 L CNN
F 1 "104" H 6900 2000 50  0000 L CNN
F 2 "lc_lib:0603_C" H 7050 2050 50  0001 C CNN
F 3 "~" H 7050 2050 50  0001 C CNN
	1    7050 2050
	1    0    0    -1  
$EndComp
Wire Wire Line
	7050 1950 6850 1950
Connection ~ 6850 1950
Wire Wire Line
	6850 1950 6850 2150
$Comp
L power:GND #PWR039
U 1 1 5CF46BCA
P 7050 2200
F 0 "#PWR039" H 7050 1950 50  0001 C CNN
F 1 "GND" H 7055 2027 50  0000 C CNN
F 2 "" H 7050 2200 50  0001 C CNN
F 3 "" H 7050 2200 50  0001 C CNN
	1    7050 2200
	1    0    0    -1  
$EndComp
Wire Wire Line
	7050 2150 7050 2200
Wire Wire Line
	8000 2700 7850 2700
Wire Wire Line
	7850 2600 7850 2700
Connection ~ 5200 3850
Connection ~ 5400 3700
Connection ~ 5400 4000
Wire Wire Line
	5200 3850 5000 3850
Wire Wire Line
	5200 3850 5600 3850
Wire Wire Line
	5200 4000 5400 4000
Wire Wire Line
	5400 4000 5700 4000
Wire Wire Line
	5200 3700 5400 3700
Wire Wire Line
	5400 3700 5950 3700
Wire Wire Line
	4800 6550 5450 6550
Wire Wire Line
	4800 6850 5450 6850
Text Label 5400 6850 0    50   ~ 0
A
Text Label 5400 6550 0    50   ~ 0
B
$Comp
L Device:R_Small R44
U 1 1 5E6321BF
P 4600 1550
F 0 "R44" V 4700 1550 50  0000 C CNN
F 1 "49.9" V 4705 1550 50  0001 C CNN
F 2 "lc_lib:0603_R" H 4600 1550 50  0001 C CNN
F 3 "~" H 4600 1550 50  0001 C CNN
	1    4600 1550
	0    1    -1   0   
$EndComp
$Comp
L Device:R_Small R45
U 1 1 5E63ACD5
P 4600 1700
F 0 "R45" V 4550 1550 50  0000 C CNN
F 1 "49.9" V 4495 1700 50  0001 C CNN
F 2 "lc_lib:0603_R" H 4600 1700 50  0001 C CNN
F 3 "~" H 4600 1700 50  0001 C CNN
	1    4600 1700
	0    1    1    0   
$EndComp
$Comp
L Device:R_Small R48
U 1 1 5E63D7B0
P 4600 1800
F 0 "R48" V 4550 1650 50  0000 C CNN
F 1 "49.9" V 4495 1800 50  0001 C CNN
F 2 "lc_lib:0603_R" H 4600 1800 50  0001 C CNN
F 3 "~" H 4600 1800 50  0001 C CNN
	1    4600 1800
	0    1    1    0   
$EndComp
$Comp
L Device:R_Small R49
U 1 1 5E63F694
P 4600 1900
F 0 "R49" V 4550 1750 50  0000 C CNN
F 1 "49.9" V 4700 1900 50  0000 C CNN
F 2 "lc_lib:0603_R" H 4600 1900 50  0001 C CNN
F 3 "~" H 4600 1900 50  0001 C CNN
	1    4600 1900
	0    1    1    0   
$EndComp
Wire Wire Line
	4700 1550 4850 1550
Connection ~ 4850 1550
Wire Wire Line
	4850 1550 4850 3400
Wire Wire Line
	4700 1700 5050 1700
Connection ~ 5050 1700
Wire Wire Line
	5050 1700 5050 1450
Wire Wire Line
	4700 1800 5150 1800
Connection ~ 5150 1800
Wire Wire Line
	5150 1800 5150 1450
Wire Wire Line
	4700 1900 5350 1900
Wire Wire Line
	5350 1450 5350 1900
Connection ~ 5350 1900
Wire Wire Line
	5350 1900 5350 3100
Wire Wire Line
	4500 1900 4350 1900
Wire Wire Line
	4350 1900 4350 1800
Wire Wire Line
	4500 1800 4350 1800
Connection ~ 4350 1800
Wire Wire Line
	4350 1800 4350 1700
Wire Wire Line
	4500 1700 4350 1700
Connection ~ 4350 1700
Wire Wire Line
	4350 1700 4350 1550
Wire Wire Line
	4500 1550 4350 1550
Connection ~ 4350 1550
Wire Wire Line
	4350 1550 4350 1500
$Comp
L power:+3.3V #PWR025
U 1 1 5E684BFA
P 4350 1350
F 0 "#PWR025" H 4350 1200 50  0001 C CNN
F 1 "+3.3V" H 4365 1523 50  0000 C CNN
F 2 "" H 4350 1350 50  0001 C CNN
F 3 "" H 4350 1350 50  0001 C CNN
	1    4350 1350
	1    0    0    -1  
$EndComp
Wire Wire Line
	5250 1450 5250 1500
Wire Wire Line
	5250 1500 4950 1500
Connection ~ 4350 1500
Wire Wire Line
	4350 1500 4350 1350
Wire Wire Line
	4950 1450 4950 1500
Connection ~ 4950 1500
Wire Wire Line
	4950 1500 4350 1500
$Comp
L Device:C_Small C12
U 1 1 5E6ADA8D
P 4200 1650
F 0 "C12" H 4050 1700 50  0000 L CNN
F 1 "104" H 4050 1600 50  0000 L CNN
F 2 "lc_lib:0603_C" H 4200 1650 50  0001 C CNN
F 3 "~" H 4200 1650 50  0001 C CNN
	1    4200 1650
	1    0    0    -1  
$EndComp
Wire Wire Line
	4200 1750 4200 1800
Wire Wire Line
	4200 1550 4350 1550
$Comp
L power:GND #PWR031
U 1 1 5E70EA97
P 4200 1800
F 0 "#PWR031" H 4200 1550 50  0001 C CNN
F 1 "GND" H 4205 1627 50  0000 C CNN
F 2 "" H 4200 1800 50  0001 C CNN
F 3 "" H 4200 1800 50  0001 C CNN
	1    4200 1800
	1    0    0    -1  
$EndComp
$Comp
L Device:C_Small C11
U 1 1 5E7126EE
P 4000 1650
F 0 "C11" H 3850 1700 50  0000 L CNN
F 1 "10uF" H 3850 1600 50  0000 L CNN
F 2 "lc_lib:0805_C" H 4000 1650 50  0001 C CNN
F 3 "~" H 4000 1650 50  0001 C CNN
	1    4000 1650
	1    0    0    -1  
$EndComp
Wire Wire Line
	4000 1550 4200 1550
Connection ~ 4200 1550
Wire Wire Line
	4000 1750 4000 1800
$Comp
L power:GND #PWR030
U 1 1 5E73114B
P 4000 1800
F 0 "#PWR030" H 4000 1550 50  0001 C CNN
F 1 "GND" H 4005 1627 50  0000 C CNN
F 2 "" H 4000 1800 50  0001 C CNN
F 3 "" H 4000 1800 50  0001 C CNN
	1    4000 1800
	1    0    0    -1  
$EndComp
Text Label 5400 3100 0    50   ~ 0
TXP
Text Label 5400 3200 0    50   ~ 0
TXN
Text Label 5400 3300 0    50   ~ 0
RXP
Text Label 5400 3400 0    50   ~ 0
RXN
$Comp
L Device:C_Small C28
U 1 1 5CB48A99
P 7450 2050
F 0 "C28" H 7300 2100 50  0000 L CNN
F 1 "104" H 7300 2000 50  0000 L CNN
F 2 "lc_lib:0603_C" H 7450 2050 50  0001 C CNN
F 3 "~" H 7450 2050 50  0001 C CNN
	1    7450 2050
	1    0    0    -1  
$EndComp
$Comp
L Device:C_Small C29
U 1 1 5CB4DCB0
P 7650 2050
F 0 "C29" H 7500 2100 50  0000 L CNN
F 1 "104" H 7500 2000 50  0000 L CNN
F 2 "lc_lib:0603_C" H 7650 2050 50  0001 C CNN
F 3 "~" H 7650 2050 50  0001 C CNN
	1    7650 2050
	1    0    0    -1  
$EndComp
$Comp
L Device:C_Small C30
U 1 1 5CB53869
P 7850 2050
F 0 "C30" H 7700 2100 50  0000 L CNN
F 1 "104" H 7700 2000 50  0000 L CNN
F 2 "lc_lib:0603_C" H 7850 2050 50  0001 C CNN
F 3 "~" H 7850 2050 50  0001 C CNN
	1    7850 2050
	1    0    0    -1  
$EndComp
Connection ~ 7050 1950
Connection ~ 7450 1950
Wire Wire Line
	7450 1950 7650 1950
Connection ~ 7650 1950
Wire Wire Line
	7650 1950 7850 1950
Connection ~ 7850 1950
Wire Wire Line
	7850 1950 8050 1950
Wire Wire Line
	8050 2150 8050 2200
Wire Wire Line
	8050 2200 7850 2200
Connection ~ 7050 2200
Wire Wire Line
	7450 2150 7450 2200
Connection ~ 7450 2200
Wire Wire Line
	7650 2150 7650 2200
Connection ~ 7650 2200
Wire Wire Line
	7650 2200 7450 2200
Wire Wire Line
	7850 2150 7850 2200
Connection ~ 7850 2200
Wire Wire Line
	7850 2200 7650 2200
Wire Wire Line
	6550 1850 6550 1900
$Comp
L Device:C_Small C14
U 1 1 5CBBD367
P 7200 1750
F 0 "C14" H 7050 1800 50  0000 L CNN
F 1 "104" H 7050 1700 50  0000 L CNN
F 2 "lc_lib:0603_C" H 7200 1750 50  0001 C CNN
F 3 "~" H 7200 1750 50  0001 C CNN
	1    7200 1750
	1    0    0    -1  
$EndComp
$Comp
L Device:C_Small C15
U 1 1 5CBBFF62
P 7350 1750
F 0 "C15" H 7200 1800 50  0000 L CNN
F 1 "104" H 7200 1700 50  0000 L CNN
F 2 "lc_lib:0603_C" H 7350 1750 50  0001 C CNN
F 3 "~" H 7350 1750 50  0001 C CNN
	1    7350 1750
	1    0    0    -1  
$EndComp
$Comp
L Device:C_Small C16
U 1 1 5CBC29F9
P 7500 1750
F 0 "C16" H 7350 1800 50  0000 L CNN
F 1 "104" H 7350 1700 50  0000 L CNN
F 2 "lc_lib:0603_C" H 7500 1750 50  0001 C CNN
F 3 "~" H 7500 1750 50  0001 C CNN
	1    7500 1750
	1    0    0    -1  
$EndComp
$Comp
L Device:C_Small C17
U 1 1 5CBD6F04
P 7650 1750
F 0 "C17" H 7500 1800 50  0000 L CNN
F 1 "104" H 7500 1700 50  0000 L CNN
F 2 "lc_lib:0603_C" H 7650 1750 50  0001 C CNN
F 3 "~" H 7650 1750 50  0001 C CNN
	1    7650 1750
	1    0    0    -1  
$EndComp
$Comp
L Device:C_Small C18
U 1 1 5CBD8B9E
P 7800 1750
F 0 "C18" H 7650 1800 50  0000 L CNN
F 1 "104" H 7650 1700 50  0000 L CNN
F 2 "lc_lib:0603_C" H 7800 1750 50  0001 C CNN
F 3 "~" H 7800 1750 50  0001 C CNN
	1    7800 1750
	1    0    0    -1  
$EndComp
$Comp
L Device:C_Small C19
U 1 1 5CBDC141
P 7950 1750
F 0 "C19" H 7800 1800 50  0000 L CNN
F 1 "104" H 7800 1700 50  0000 L CNN
F 2 "lc_lib:0603_C" H 7950 1750 50  0001 C CNN
F 3 "~" H 7950 1750 50  0001 C CNN
	1    7950 1750
	1    0    0    -1  
$EndComp
$Comp
L Device:C_Small C20
U 1 1 5CBDEEA8
P 8100 1750
F 0 "C20" H 7950 1800 50  0000 L CNN
F 1 "104" H 7950 1700 50  0000 L CNN
F 2 "lc_lib:0603_C" H 8100 1750 50  0001 C CNN
F 3 "~" H 8100 1750 50  0001 C CNN
	1    8100 1750
	1    0    0    -1  
$EndComp
$Comp
L Device:C_Small C21
U 1 1 5CC16756
P 8250 1750
F 0 "C21" H 8100 1800 50  0000 L CNN
F 1 "104" H 8100 1700 50  0000 L CNN
F 2 "lc_lib:0603_C" H 8250 1750 50  0001 C CNN
F 3 "~" H 8250 1750 50  0001 C CNN
	1    8250 1750
	1    0    0    -1  
$EndComp
$Comp
L Device:C_Small C31
U 1 1 5CC55A09
P 8050 2050
F 0 "C31" H 7900 2100 50  0000 L CNN
F 1 "104" H 7900 2000 50  0000 L CNN
F 2 "lc_lib:0603_C" H 8050 2050 50  0001 C CNN
F 3 "~" H 8050 2050 50  0001 C CNN
	1    8050 2050
	1    0    0    -1  
$EndComp
Wire Wire Line
	7050 1650 7200 1650
Connection ~ 7200 1650
Wire Wire Line
	7200 1650 7350 1650
Connection ~ 7350 1650
Wire Wire Line
	7350 1650 7500 1650
Connection ~ 7500 1650
Wire Wire Line
	7500 1650 7650 1650
Connection ~ 7650 1650
Wire Wire Line
	7650 1650 7800 1650
Connection ~ 7800 1650
Wire Wire Line
	7800 1650 7950 1650
Connection ~ 7950 1650
Wire Wire Line
	7950 1650 8100 1650
Connection ~ 8100 1650
Wire Wire Line
	8100 1650 8250 1650
Wire Wire Line
	7050 1850 7050 1900
Wire Wire Line
	7050 1900 7200 1900
Wire Wire Line
	8400 1900 8400 1950
$Comp
L power:GND #PWR035
U 1 1 5CC850F3
P 8400 1950
F 0 "#PWR035" H 8400 1700 50  0001 C CNN
F 1 "GND" H 8405 1777 50  0000 C CNN
F 2 "" H 8400 1950 50  0001 C CNN
F 3 "" H 8400 1950 50  0001 C CNN
	1    8400 1950
	1    0    0    -1  
$EndComp
Wire Wire Line
	8250 1850 8250 1900
Connection ~ 8250 1900
Wire Wire Line
	8250 1900 8400 1900
Wire Wire Line
	8100 1850 8100 1900
Connection ~ 8100 1900
Wire Wire Line
	8100 1900 8250 1900
Wire Wire Line
	7950 1850 7950 1900
Connection ~ 7950 1900
Wire Wire Line
	7950 1900 8100 1900
Wire Wire Line
	7800 1850 7800 1900
Connection ~ 7800 1900
Wire Wire Line
	7800 1900 7950 1900
Wire Wire Line
	7650 1850 7650 1900
Connection ~ 7650 1900
Wire Wire Line
	7650 1900 7800 1900
Wire Wire Line
	7500 1850 7500 1900
Connection ~ 7500 1900
Wire Wire Line
	7500 1900 7650 1900
Wire Wire Line
	7350 1850 7350 1900
Connection ~ 7350 1900
Wire Wire Line
	7350 1900 7500 1900
Wire Wire Line
	7200 1850 7200 1900
Connection ~ 7200 1900
Wire Wire Line
	7200 1900 7350 1900
Wire Wire Line
	7050 1650 6650 1650
Wire Wire Line
	6650 1650 6650 1900
Wire Wire Line
	6650 1900 6550 1900
Connection ~ 7050 1650
Connection ~ 6550 1900
Wire Wire Line
	6550 1900 6550 2150
Text Label 5700 3700 0    50   ~ 0
X+
Text Label 5700 3800 0    50   ~ 0
X-
Text Label 2550 4150 0    50   ~ 0
SPI_CS
Text Label 7500 2900 0    50   ~ 0
SPI_CS
Wire Wire Line
	7450 2900 7500 2900
Wire Wire Line
	7200 800  7700 800 
$Comp
L MCU_ST_STM32F0:STM32F030C8Tx U5
U 1 1 602E403F
P 1950 3850
F 0 "U5" H 1950 2161 50  0000 C CNN
F 1 "STM32F030C8Tx" H 1950 2070 50  0000 C CNN
F 2 "lc_lib:LQFP-48_7X7X05P" H 1450 2350 50  0001 R CNN
F 3 "http://www.st.com/st-web-ui/static/active/en/resource/technical/document/datasheet/DM00088500.pdf" H 1950 3850 50  0001 C CNN
	1    1950 3850
	1    0    0    -1  
$EndComp
Wire Wire Line
	1100 2700 1100 2650
Wire Wire Line
	1100 2650 1350 2650
$Comp
L power:GND #PWR043
U 1 1 6035D887
P 1100 3000
F 0 "#PWR043" H 1100 2750 50  0001 C CNN
F 1 "GND" H 1105 2827 50  0000 C CNN
F 2 "" H 1100 3000 50  0001 C CNN
F 3 "" H 1100 3000 50  0001 C CNN
	1    1100 3000
	1    0    0    -1  
$EndComp
Wire Wire Line
	1100 2900 1100 3000
Wire Wire Line
	2550 5050 2800 5050
Wire Wire Line
	2550 5150 2700 5150
Wire Wire Line
	800  2700 800  2450
Wire Wire Line
	800  2450 1350 2450
$Comp
L power:GND #PWR042
U 1 1 603EDDF7
P 800 3000
F 0 "#PWR042" H 800 2750 50  0001 C CNN
F 1 "GND" H 805 2827 50  0000 C CNN
F 2 "" H 800 3000 50  0001 C CNN
F 3 "" H 800 3000 50  0001 C CNN
	1    800  3000
	1    0    0    -1  
$EndComp
Wire Wire Line
	800  2900 800  3000
$Comp
L Device:C_Small C22
U 1 1 60467F1F
P 1600 2000
F 0 "C22" H 1692 2046 50  0000 L CNN
F 1 "104" H 1692 1955 50  0000 L CNN
F 2 "lc_lib:0603_C" H 1600 2000 50  0001 C CNN
F 3 "~" H 1600 2000 50  0001 C CNN
	1    1600 2000
	-1   0    0    -1  
$EndComp
$Comp
L Device:C_Small C23
U 1 1 604744E2
P 1800 2000
F 0 "C23" H 1892 2046 50  0000 L CNN
F 1 "104" H 1892 1955 50  0000 L CNN
F 2 "lc_lib:0603_C" H 1800 2000 50  0001 C CNN
F 3 "~" H 1800 2000 50  0001 C CNN
	1    1800 2000
	-1   0    0    -1  
$EndComp
$Comp
L Device:C_Small C24
U 1 1 60479625
P 1950 2000
F 0 "C24" H 2042 2046 50  0000 L CNN
F 1 "104" H 2042 1955 50  0000 L CNN
F 2 "lc_lib:0603_C" H 1950 2000 50  0001 C CNN
F 3 "~" H 1950 2000 50  0001 C CNN
	1    1950 2000
	-1   0    0    -1  
$EndComp
$Comp
L Device:C_Small C25
U 1 1 6047EB7B
P 2150 2000
F 0 "C25" H 2242 2046 50  0000 L CNN
F 1 "104" H 2242 1955 50  0000 L CNN
F 2 "lc_lib:0603_C" H 2150 2000 50  0001 C CNN
F 3 "~" H 2150 2000 50  0001 C CNN
	1    2150 2000
	-1   0    0    -1  
$EndComp
Connection ~ 1800 2100
Wire Wire Line
	1800 2100 1600 2100
Connection ~ 1950 2100
Wire Wire Line
	1950 2100 1800 2100
Wire Wire Line
	2150 2100 1950 2100
Wire Wire Line
	1600 1900 1800 1900
Wire Wire Line
	2350 1900 2350 2150
Connection ~ 1800 1900
Wire Wire Line
	1800 1900 1950 1900
Connection ~ 1950 1900
Wire Wire Line
	1950 1900 2150 1900
Connection ~ 2150 1900
Wire Wire Line
	2150 1900 2350 1900
$Comp
L power:GND #PWR036
U 1 1 604B17E4
P 1600 2150
F 0 "#PWR036" H 1600 1900 50  0001 C CNN
F 1 "GND" H 1605 1977 50  0000 C CNN
F 2 "" H 1600 2150 50  0001 C CNN
F 3 "" H 1600 2150 50  0001 C CNN
	1    1600 2150
	1    0    0    -1  
$EndComp
Wire Wire Line
	1600 2150 1600 2100
Connection ~ 1600 2100
Wire Wire Line
	2150 1900 2150 1750
$Comp
L power:+3.3V #PWR029
U 1 1 604D85C9
P 2150 1750
F 0 "#PWR029" H 2150 1600 50  0001 C CNN
F 1 "+3.3V" H 2165 1923 50  0000 C CNN
F 2 "" H 2150 1750 50  0001 C CNN
F 3 "" H 2150 1750 50  0001 C CNN
	1    2150 1750
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR049
U 1 1 6050D585
P 2500 5750
F 0 "#PWR049" H 2500 5500 50  0001 C CNN
F 1 "GND" H 2505 5577 50  0000 C CNN
F 2 "" H 2500 5750 50  0001 C CNN
F 3 "" H 2500 5750 50  0001 C CNN
	1    2500 5750
	1    0    0    -1  
$EndComp
Text GLabel 1350 4050 0    50   Output ~ 0
BCD1_1
Text GLabel 2600 5300 3    50   Output ~ 0
BCD1_2
Text GLabel 2550 4950 2    50   Output ~ 0
BCD2_0
Text GLabel 2550 4850 2    50   Output ~ 0
BCD2_1
Text GLabel 2550 4750 2    50   Output ~ 0
BCD2_2
Text GLabel 2550 4650 2    50   Input ~ 0
BCDM1_0
Text GLabel 2550 4550 2    50   Input ~ 0
BCDM1_1
Text GLabel 1350 5250 0    50   Input ~ 0
BCDM1_2
Text GLabel 1350 5150 0    50   Input ~ 0
BCDM2_0
Text GLabel 1350 5050 0    50   Input ~ 0
BCDM2_1
Text GLabel 1350 4950 0    50   Input ~ 0
BCDM2_2
Wire Wire Line
	2550 5250 2600 5250
Wire Wire Line
	2600 5250 2600 5300
Wire Wire Line
	2550 3850 3100 3850
Wire Wire Line
	3100 3850 3100 5150
Text Label 1250 4850 2    50   ~ 0
485RW
Wire Wire Line
	1250 4850 1350 4850
Wire Wire Line
	5400 5150 7900 5150
Wire Wire Line
	3100 5150 5200 5150
$Comp
L Device:R_Small R52
U 1 1 5CFA0F72
P 5300 5150
F 0 "R52" V 5500 5300 50  0000 C CNN
F 1 "33" V 5400 5300 50  0000 C CNN
F 2 "lc_lib:0603_R" H 5300 5150 50  0001 C CNN
F 3 "~" H 5300 5150 50  0001 C CNN
	1    5300 5150
	0    1    1    0   
$EndComp
Wire Wire Line
	1850 5450 1850 5550
Wire Wire Line
	1850 5550 1950 5550
Wire Wire Line
	2500 5550 2500 5750
Wire Wire Line
	1950 5450 1950 5550
Connection ~ 1950 5550
Wire Wire Line
	1950 5550 2050 5550
Wire Wire Line
	2050 5450 2050 5550
Connection ~ 2050 5550
Wire Wire Line
	2050 5550 2500 5550
Wire Wire Line
	1850 2250 1850 2150
Wire Wire Line
	1950 2250 1950 2150
Wire Wire Line
	1850 2150 1950 2150
Connection ~ 1950 2150
Wire Wire Line
	1950 2150 2050 2150
Wire Wire Line
	2050 2250 2050 2150
Connection ~ 2050 2150
Wire Wire Line
	2050 2150 2150 2150
Wire Wire Line
	2150 2250 2150 2150
Connection ~ 2150 2150
Wire Wire Line
	2150 2150 2350 2150
$Comp
L power:GND #PWR0102
U 1 1 5DBB73F9
P 4900 3900
F 0 "#PWR0102" H 4900 3650 50  0001 C CNN
F 1 "GND" H 4905 3727 50  0000 C CNN
F 2 "" H 4900 3900 50  0001 C CNN
F 3 "" H 4900 3900 50  0001 C CNN
	1    4900 3900
	1    0    0    -1  
$EndComp
$Comp
L Connector_Generic:Conn_01x01 J2
U 1 1 5E3B6863
P 800 1950
F 0 "J2" V 764 1862 50  0000 R CNN
F 1 "Conn_01x01" V 673 1862 50  0000 R CNN
F 2 "Connectors:PINTST" H 800 1950 50  0001 C CNN
F 3 "~" H 800 1950 50  0001 C CNN
	1    800  1950
	0    -1   -1   0   
$EndComp
Wire Wire Line
	800  2150 800  2450
Connection ~ 800  2450
Wire Wire Line
	7050 1950 7450 1950
Wire Wire Line
	7050 2200 7450 2200
$Comp
L MySchLib:HR913550A J5
U 1 1 5F3FE682
P 4800 1350
F 0 "J5" V 5038 -128 50  0000 R CNN
F 1 "HR913550A" V 4947 -128 50  0000 R CNN
F 2 "MyPCBLib:HR913550A" V 5350 650 50  0000 C CNN
F 3 "" H 4800 1350 50  0001 C CNN
	1    4800 1350
	0    -1   -1   0   
$EndComp
Wire Wire Line
	6150 1450 6150 2300
Wire Wire Line
	6150 2300 5850 2300
Connection ~ 5850 2300
Wire Wire Line
	5850 2300 5850 2500
Wire Wire Line
	8350 800  8600 800 
$Comp
L Regulator_Linear:SPX3819M5-L-1-8 U3
U 1 1 650B1186
P 8050 900
F 0 "U3" H 8050 1242 50  0000 C CNN
F 1 "SPX3819M5-L-1-8" H 8050 1151 50  0000 C CNN
F 2 "Package_TO_SOT_SMD:SOT-23-5" H 8050 1225 50  0001 C CNN
F 3 "https://www.exar.com/content/document.ashx?id=22106&languageid=1033&type=Datasheet&partnumber=SPX3819&filename=SPX3819.pdf&part=SPX3819" H 8050 900 50  0001 C CNN
	1    8050 900 
	1    0    0    -1  
$EndComp
Wire Wire Line
	7750 900  7700 900 
Connection ~ 7700 900 
Text GLabel 950  4750 0    50   Output ~ 0
XDISPLAY
Wire Wire Line
	950  4750 1350 4750
$EndSCHEMATC
